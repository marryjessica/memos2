 ...
 */
const DailyMemoGroup = ({ group, renderMemo, onQuickAdd, className, isToday = false, defaultExpanded = true }: DailyMemoGroupProps) => {
    ...
    >
      ...
      >
        <div className="flex items-center gap-2">
          ...
          <button className="p-0.5 hover:bg-accent rounded transition-colors" aria-label={isExpanded ? "收起" : "展开"}>
            {isExpanded ? (
            ...
            <Tooltip>
              <TooltipTrigger asChild>
                <div className="flex items-center gap-2">
                  ...
                  <span className={cn("font-medium text-sm", isToday ? "text-primary" : "text-foreground")}>{group.displayDate}</span>
                </div>
              </TooltipTrigger>
              <TooltipContent>
                <p>{group.fullDate}</p>
          ...
          {/* Memo Count Badge */}
          <span className="text-xs text-muted-foreground bg-muted px-1.5 py-0.5 rounded">{group.memos.length} 条</span>
        </div>
        ...
        <div className="flex items-center gap-3">
          ...
          {hasTaskStats && (
            <div className="flex items-center gap-2 text-xs text-muted-foreground">
              {group.completeCount > 0 && <span className="text-green-600 dark:text-green-400">✓ {group.completeCount}</span>}
              {group.incompleteCount > 0 && <span className="text-amber-600 dark:text-amber-400">○ {group.incompleteCount}</span>}
            </div>
          )}
          ...
          <Button variant="ghost" size="sm" className="h-7 px-2" onClick={handleQuickAddClick}>
      ...
      {isExpanded && (
        <div className="px-4 pb-3 space-y-0">
          ...
          {group.memos.length === 0 && !showQuickAddEditor ? (
            <div className="text-center py-6 text-muted-foreground text-sm">暂无待办</div>
          ) : (
            group.memos.map((memo) => <div key={memo.name}>{renderMemo(memo)}</div>)
          )}
        </div>
      )}
    </div>
