 ...
 */
const MemoAnnotationPanel = ({ memo, open, onClose, className }: MemoAnnotationPanelProps) => {
  const [newAnnotation, setNewAnnotation] = useState("");
  const [showEditor, setShowEditor] = useState(false);

  const { annotations, count, isLoading, addAnnotation, deleteAnnotation, isAdding, isDeleting } = useAnnotations(memo.name);
    ...
    >
      ...
      <div className="flex items-center justify-between px-4 py-3 border-b border-border">
        <div className="flex items-center gap-2">
          ...
          {count > 0 && <span className="text-xs text-muted-foreground bg-muted px-1.5 py-0.5 rounded">{count}</span>}
        </div>
        <div className="flex items-center gap-1">
          <Button variant="ghost" size="sm" className="h-7 w-7 p-0" onClick={() => setShowEditor(true)}>
      ...
      {showEditor && (
        <div className="p-3 border-b border-border bg-muted/30">
          ...
          <div className="flex justify-end gap-2 mt-2">
            ...
            <Button size="sm" onClick={handleAddAnnotation} disabled={isAdding || !newAnnotation.trim()}>
      ...
      <div className="flex-1 overflow-y-auto">
        {isLoading ? (
          ...
          <div className="flex flex-col items-center justify-center py-12 text-center">
            ...
            <Button variant="ghost" size="sm" className="mt-2" onClick={() => setShowEditor(true)}>
...
const AnnotationCard = ({ annotation, onDelete, isDeleting }: AnnotationCardProps) => {
  ...
  const timeStr = annotation.createTime ? dayjs(timestampDate(annotation.createTime)).format("M/D HH:mm") : "";
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center justify-between mb-2">
        ...
        {showDelete && (
          <Button variant="ghost" size="sm" className="h-6 w-6 p-0 opacity-60 hover:opacity-100" onClick={onDelete} disabled={isDeleting}>
