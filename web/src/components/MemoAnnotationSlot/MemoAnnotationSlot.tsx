...
const MemoAnnotationSlot = ({ memo, className, isFocused, onFocusDismiss }: MemoAnnotationSlotProps) => {
  ...
  const { annotations, addAnnotation, updateAnnotation, deleteAnnotation, isAdding, isUpdating, isDeleting } = useAnnotations(memo.name);
        ...
        <div className="relative group">
          ...
          <div className="bg-muted/30 rounded-lg border border-border/50 focus-within:border-primary/30 focus-within:bg-muted/50 transition-all">
            ...
            <div className="flex justify-end gap-2 p-2 pt-0">
              ...
              <Button size="sm" onClick={handleAddAnnotation} disabled={isAdding || !newAnnotation.trim()} className="h-6 px-3 text-xs">
                发送
              </Button>
            </div>
          </div>
              ...
              {editingAnnotationId === annotation.name ? (
                <div className="bg-muted/30 rounded-lg border border-primary/20 p-2">
                  ...
                  <div className="flex justify-end gap-2 pt-2">
                    <Button variant="ghost" size="sm" onClick={() => setEditingAnnotationId(null)} className="h-5 px-2 text-xs">
                      取消
                    </Button>
                    <Button
                      size="sm"
